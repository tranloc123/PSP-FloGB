...
if s.count(anchor) != 1:
    raise SystemExit(f"V0.5.4a Top5 anchor count = {s.count(anchor)}")
...
for required in (
    ...
    if required not in s:
        raise SystemExit(f"V0.5.4a generated source missing: {required}")

debug.write_text(s, encoding="utf-8")

with info.open("a", encoding="utf-8") as f:
    f.write(
        "\nPSP Live FloGB V0.5.4a Tier Rank Avatar Frames\n"
...
print("PSP Live FloGB V0.5.4a Tier Rank Frames patch applied successfully.")
